...
#FIXME COPY ALL YOUR MATRICES AND INPUTS BEFORE RETURNING THEM
    #####################
    #   STATIC METHODS  #
    #####################
    @staticmethod
    def list_to_vec(input:list, column_vector:bool=True):
        #vector list
        new_matrix = []
        assert type(input) == list, "ERROR: input is not list"
        assert Matrix.can_vector(input), "ERROR: input contents are not compatible to vector"
        #if column vec
        if column_vector:
            for i in input:
                new_matrix.append([i])
            matrix_obj = Matrix(len(input), 1)
            matrix_obj.matrix = new_matrix
            return matrix_obj
        #if row vec
        else:
            new_matrix.append(list(input))
            matrix_obj = Matrix(1, len(input))
            matrix_obj.matrix = new_matrix
            return matrix_obj

    #FIXME
    @staticmethod
    def list_to_matrix(input:list):
        new_matrix = []
        assert Matrix.can_matrix(input), "ERROR: input is not compatible to matrix"
        for row in range(len(input)):
            new_matrix.append(list(input[row]))

            matrix_obj = Matrix(len(input), len(input[0]))
            matrix_obj.matrix = new_matrix
            return matrix_obj
    ...
    def add_matrix(matrix, input):
        ...
        #make copy of matrix
        matrix = Matrix.deepcopy_matrix(matrix)
        
        #add matrix
        if type(input) == Matrix:
            #make copy of input matrix
            input = Matrix.deepcopy_matrix(input)
            #check if matrices are compatible
            assert Matrix.is_same_dimensions(matrix, input), "ERROR: Input matrix not compatible to add"
            
            #add matrices
            for row in range(matrix.rows):
                new_row = []
                for col in range(matrix.columns):
                    new_row.append(matrix.matrix[row][col] + input.matrix[row][col])
                new_matrix.append(new_row)
        ...
        #add scalar
            ...
            for row in range(matrix.rows):
                new_row = []
                for col in range(matrix.columns):
                    new_row.append(matrix.matrix[row][col] + input)
                new_matrix.append(new_row)
        ...
        matrix_obj = Matrix.from_list(new_matrix)
        return matrix_obj
    ...
    def subtract_matrix(matrix, input):
        ...
        if type(input) == Matrix:
            ...
            input = Matrix.deepcopy_matrix(input)
            #check if matrices are compatible
            assert Matrix.is_same_dimensions(matrix, input), "ERROR: Input matrix not compatible to subtract"
    ...
    def multiply_matrix(matrix, input, by_element:bool = False):
        ...
        assert type(matrix) == Matrix, "Matrix not a matrix"
        ...
        if type(input) == Matrix:
            ...
            if by_element:
                ...
                assert Matrix.is_same_dimensions(matrix, input), "ERROR: input is not same size; unable to multiply element-wise"
                ...
                #ensure matrices are compatible
                assert Matrix.can_multiply_matrices(matrix, input), "ERROR: incompatible sizes of matrices to multiply"
                #copy input matrix
                input = Matrix.deepcopy_matrix(input)
                #get new matrix dimensions
                rows = len(matrix.matrix)
                cols = len(input.matrix[0])
                #for looping purposes, get the common dimension
                comp_numb = len(input.matrix)
                ...
                for row in range(rows):
                    ...
                    for col in range(cols):
                        ...
                        for j in range(comp_numb):
                            sum_prod += (matrix.matrix[row][j] * input.matrix[j][col])
                        new_row.append(sum_prod)
                    new_matrix.append(new_row)
        ...
        matrix_obj = Matrix.list_to_matrix(new_matrix)
        return matrix_obj
    ...
    def from_map(matrix, func):
        ...
        for row in range(matrix.rows):
            new_row = []
            for col in range(matrix.columns):
                new_row.append(func(matrix.matrix[row][col]))
            new_matrix.append(new_row)
        matrix_obj = Matrix.from_list(new_matrix)
        return matrix_obj
    ...
    def get_row_percentage(matrix):
        ...
        for row in range(matrix.rows):
            row_sum = 0
            for col in range(matrix.columns):
                row_sum += matrix.matrix[row][col]
            row_sums_list.append(row_sum)
        #make new matrix with pergentaged values
        for row in range(matrix.rows):
            new_row = []
            for col in range(matrix.columns):
                new_row.append(matrix.matrix[row][col] / row_sums_list[row])
            new_matrix.append(new_row)
    ...
    def sum_of_squared_diff_cost(output_vector:list, target_vector:list) -> float:
        cost = 0.0
        vec_diff = Matrix.subtract(output_vector, target_vector)
    ...
    def _ut_multiplication() -> str:
        ...
        #run test
        c1 = Matrix.multiply(a1,b1)
        c2 = Matrix.multiply(a2,b2)
        if not c1 == c_sol1: return "ERROR: Fail in case 1"
        if not c2 == c_sol2: return "ERROR: Fail in case 2"
        return "SUCCESS: Both cases OK in - unit test matrix multiplication"

    def _ut_transpose() -> str:
        #first test
        a1 = [
            [2,3,5,6],
            [1,4,11,6],
        ...
        b1 = Matrix.transpose(a1)
        if not b1 == b_sol1: return "ERROR: Fail in case 1"
        return "SUCCESS: All cases OK in - unit test matrix transpose"
    ...
    def _ut_map() -> str:
        ...
        #run test
        b1 = Matrix.apply_map(Relu.function,a1)
        if not b1 == b_sol1: return "ERROR: fail in case 1"
        return "SUCCESS: All cases OK in - unit test matrix apply map"
    
    def _ut_same_dimensions() -> str:
        #first case
        a1 = [
            [0,0],
            [0,0]
        ...
        if not Matrix.is_same_dimensions(a1,b1): return "ERROR: fail at first case"
        return "SUCCESS: All cases OK in - unit test same dimensions"
